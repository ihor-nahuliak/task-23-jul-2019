...
class WalletsRepo(AbstractRepo):
    entity_class = Wallet
    ...
    @classmethod
    def _sa_wallets(cls, filter_params=None):
    ...
    def _project_clause(cls, project_params=None):
        ...
        columns = {
            'id': tbl_wallets.c.id,
            'is_enabled': tbl_wallets.c.is_enabled,
            'created_at': tbl_wallets.c.created_at,
            'updated_at': tbl_wallets.c.updated_at,
            'client_id': tbl_wallets.c.client_id,
            'currency': tbl_wallets.c.currency,
            'collected_balance': tbl_wallets.c.collected_balance,
            # 'in_queue_balance':
            #     sa.func.sum(tbl_payments.amount).label('in_queue_balance'),
        }
    ...
    def _where_clause(cls, filter_params=None):
        clause = sa.and_(
            tbl_wallets.c.is_enabled.is_(True)
        )
        ...
        if filter_params.id:
            clause = sa.and_(
                clause,
                tbl_wallets.c.id == filter_params.id
            )
        elif filter_params.id__in:
            clause = sa.and_(
                clause,
                tbl_wallets.c.id.in_(filter_params.id__in)
            )
        if filter_params.client_id:
            clause = sa.and_(
                clause,
                tbl_wallets.c.client_id == filter_params.client_id
            )
        elif filter_params.client_id__in:
            clause = sa.and_(
                clause,
                tbl_wallets.c.client_id.in_(filter_params.client_id__in)
            )
        if filter_params.currency:
            clause = sa.and_(
                clause,
                tbl_wallets.c.currency == filter_params.currency
            )
        elif filter_params.currency__in:
            clause = sa.and_(
                clause,
                tbl_wallets.c.currency.in_(filter_params.currency__in)
            )
    ...
    def _order_clause(cls, sorting_params=None):
        ...
        clause = (tbl_wallets.c.id,)
        return clause

    async def get_count(self, filter_params=None):
        query = sa.select([
            sa.func.count(tbl_wallets.c.id).label('count')
        ]).select_from(
            self._sa_wallets(filter_params)
        ).where(
    ...
    async def create_list(self, items_list):
        await self._conn.execute(
            ...
            [
                {
                    tbl_wallets.c.id: item.id,
                    tbl_wallets.c.is_enabled: True,
                    tbl_wallets.c.created_at: item.created_at,
                    tbl_wallets.c.updated_at: None,
                    tbl_wallets.c.client_id: item.client_id,
                    tbl_wallets.c.currency: item.currency,
                    tbl_wallets.c.collected_balance: 0,
                }
